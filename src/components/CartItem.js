...
const CartItem = ({item} ) => {
  ...
  return <div>
    <div className='w-full min-h-[150px] flex items-center gap-x-4 py-2 lg:px-6 border-b border-gray-200 w-full font-light text-gray-500  '>
      {/*image*/}
      <Link to={`/product/${id}`}>
        <img className='max-w-[80px] ' src={productImg} alt=''/>
      </Link>
      <div className='w-full flex flex-col'>
        {/*title and remove icon */}
        <div  onClick={() => removeFromCart(id)}  className='flex justify-between mb-2'>
          <Link to={`/product/${id}`} className='text-sm uppercase font-medium max-u-[240px] text-primary hover:underline'>{title}</Link>
         {/* remove icon*/} 
